    ...
    def test_official_corpus_metadata_gaps_are_registered_raw_or_typed(self) -> None:
        ...
        for (profile, scope), meta_ids in coverage.items():
            for meta_id in meta_ids:
                with self.subTest(profile=profile, scope=scope, meta_id=meta_id):
                    self.assertIsNotNone(metadata_definition(profile, scope, meta_id))

        self.assertEqual(
            metadata_definition("fiesta2", MetadataScope.HEADER, 66638).kind,
            ValueKind.TRAILER_OFFSET,
        )
        self.assertEqual(
            metadata_definition("prime2", MetadataScope.HEADER, 328783).kind,
            ValueKind.TRAILER_OFFSET,
        )
